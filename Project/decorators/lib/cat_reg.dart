...
class CatReg extends StatefulWidget {
  const CatReg({super.key});

  @override
  State<CatReg> createState() => _CatRegState();
}

class _CatRegState extends State<CatReg> {
  ...
  void insert(final id) async {
    try {
      ...
      if (url!.isNotEmpty) {
        await supabase.from('tbl_catering').insert({
          'id': id,
          'cat_name': name,
          'cat_email': email,
          'cat_contact': contact,
  ...
  Future<String?> photoUpload(String uid, PlatformFile file) async {
    try {
      final bucketName = 'catering'; // Replace with your bucket name
